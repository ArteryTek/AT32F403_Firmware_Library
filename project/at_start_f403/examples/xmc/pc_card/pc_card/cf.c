...
uint8_t *read_sector(pccard_handle_type *hpccard, uint8_t *pbuffer, uint32_t transfer_size)
{
  ...
  if(hpccard->cf.dma_enable == TRUE)
  {
    ...
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)hpccard->cf.io_addr, (uint32_t)pbuffer, transfer_size, READ_TRANSFER, ENABLE_8_BIT_TRANSFER);
    }
    else
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)hpccard->cf.io_addr, (uint32_t)pbuffer, (transfer_size / 2), READ_TRANSFER, ENABLE_16_BIT_TRANSFER);
    }
...
uint8_t *write_sector(pccard_handle_type *hpccard, uint8_t *pbuffer, uint32_t transfer_size)
{
  ...
  if(hpccard->cf.dma_enable == TRUE)
  {
    if(hpccard->cf.enable_8_bit_mode)
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)pbuffer, (uint32_t)hpccard->cf.io_addr, transfer_size, WRITE_TRANSFER, ENABLE_8_BIT_TRANSFER);
    }
    else
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)pbuffer, (uint32_t)hpccard->cf.io_addr, (transfer_size / 2), WRITE_TRANSFER, ENABLE_16_BIT_TRANSFER);
    }
